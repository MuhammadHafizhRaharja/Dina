...
if ($search === '') {
    ...
    $sql  = "SELECT nama_wisata, deskripsi, foto_profil, link_maps, link_detail
             FROM wisata
             WHERE LOWER(city) = ? AND (LOWER(nama_wisata) LIKE ? OR LOWER(deskripsi) LIKE ?)
             ORDER BY nama_wisata ASC";
    $like = "%$search%";
    $stmt = $conn->prepare($sql);
    $cityParam = strtolower($city);
    $stmt->bind_param('sss', $cityParam, $like, $like);
}
...
<div class="search">
    ...
    <form class="search-bar" method="get" action="tourismbandung.php">
        <i class="fas fa-search"></i>
        <input type="text" name="search"
            value="<?= htmlspecialchars($search) ?>" placeholder="Search destinations in Bandung...">
